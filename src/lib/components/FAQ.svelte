...
<section id="faq" class="bg-primary-50 px-6 py-24">
  <div class="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12">
    <div class="lg:col-span-4">
      <h2 class="font-serif text-4xl text-primary-900 mb-6">
        Common questions
      </h2>
      <p class="text-primary-600 mb-4 leading-relaxed">Still have questions? We're here to help.</p>
      <a href="#contact" class="text-sm text-primary-900 border-b border-primary-300 pb-0.5 hover:border-primary-700 transition-colors">Get in touch</a>
    </div>
      ...
      <div class="border-t border-primary-200">
        {#each faqs as faq, i}
          <div class="border-b border-primary-200">
            <button 
              class="w-full py-6 flex justify-between items-center text-left hover:bg-white transition-colors"
              on:click={() => toggle(i)}
            >
              <span class="text-lg text-primary-900">{faq.question}</span>
              <span class="text-2xl font-light text-primary-500 ml-4">
                {openIndex === i ? '−' : '+'}
              </span>
            </button>
            {#if openIndex === i}
              <div transition:slide class="pb-6 text-primary-700 leading-relaxed">
                {faq.answer}
              </div>
            {/if}
          </div>
